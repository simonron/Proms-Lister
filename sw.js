const CACHE='proms-lister-version-3.0.22';
...
function refresh(cache,request){
  fetch(new Request(request,{cache:'no-cache'})).then(response=>put(cache,request,response)).catch(()=>{});
}
...
self.addEventListener('fetch',event=>{
  ...
  event.respondWith((async()=>{
    ...
    if(cdn){
      ...
      if(hit){refresh(cache,event.request);return hit}
    ...
    if(exact){refresh(cache,event.request);return exact}
