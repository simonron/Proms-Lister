...
function locate(t){const k=key(t&&t.section,t&&t.row,t&&t.seat);return load()[k]||LOOKUP[k]||null}
...
function currentTicket(){const live={door:field('door'),section:field('section'),row:field('row'),seat:field('seat')};if(live.section&&live.row&&live.seat)return live;return activeTicket||{}}
...
function labelTicket(t,p){const label=document.getElementById('seatLabel');if(!label)return;const d=[t.door&&'Door '+t.door,t.section,t.row&&'Row '+t.row,t.seat&&'Seat '+t.seat].filter(Boolean).join(' · ');label.textContent=d+(p?' — '+p.kind:(d?' — position not yet indexed':''))}
